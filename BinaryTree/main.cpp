...
int main()
{
    ...
    c_start = clock();
    cout << "Среднее арефметическое Бинарного дерева: " << tree.Avg(tree.getRoot()) << endl;
    c_end = clock();
    cout << "  : " << double(c_end - c_start) / CLOCKS_PER_SEC << endl;

    c_start = clock();
    cout << "Глубина Бинарного дерева: " << tree.Depth(tree.getRoot()) << endl;
    c_end = clock();
    cout << "  : " << double(c_end - c_start) / CLOCKS_PER_SEC << endl;
    UniqueTree u_tree;
    for (size_t i = 0; i < n; i++)
    ...
    cout << endl;
    cout << "Количество элементов Бинарного дерева: " << u_tree.Count(u_tree.getRoot()) << endl;
#endif // HW_1
